...
export class MfaVerificationController {
  ...
  async verifyTotp(
    ...
    const { response, refreshToken } = await this.mfaVerificationService.verifyTotp(dto.sessionId, dto.code);
  ...
  async verifySmsOtp(
    ...
    const { response, refreshToken } = await this.mfaVerificationService.verifySmsOtp(dto.sessionId, dto.code);
  ...
  async verifyPasskeyMfa(
    ...
    const { response, refreshToken } = await this.mfaVerificationService.verifyPasskeyMfa(dto.sessionId, dto.credential as any);
