 ...
 */
@Controller('onboarding/webhooks/whatsapp')
export class WhatsAppWebhookController {
  ...
  constructor(
    ...
    if (!this.verifyToken) {
      this.logger.warn('WHATSAPP_VERIFY_TOKEN is not configured. Webhook verification will fail.');
    }
  }
  ...
  private async processWebhookAsync(payload: WhatsAppWebhookDto): Promise<void> {
    try {
      ...
      for (const entry of payload.entry || []) {
        for (const change of entry.changes || []) {
          ...
          for (const message of messages) {
            ...
            this.logger.log(`Processing message from ${senderName} (${phoneNumber}): "${messageText}"`);
      ...
      this.logger.error(`Error in webhook processing: ${error.message}`, error.stack);
      throw error;
    }
  }
